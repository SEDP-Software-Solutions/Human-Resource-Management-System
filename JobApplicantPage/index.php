    ...
    <div class="container bg-light my-2 shadow rounded">
        <div class="row">
            <nav aria-label="breadcrumb" class="my-2 d-flex justify-content-between">
                ...
                <a href="./View/JobApplicantStatus.php" class="text-decoration-none">Job Status</a>
            </nav>
    ...
    < <section>
        <div class="container mb-5 mt-4 bg-light p-3">
                ...
                <?php foreach ($jobOffers as $row): ?>
                    <div class='col-lg-5 mx-2 mb-3'>
                        <div class='card mb-3 shadow'>
                            <div class='card-body'>
                                <h5 class='card-title'><i class='bi bi-briefcase-fill'></i> <?= htmlspecialchars($row['title']) ?></h5>
                                <p class='card-text mx-2'><i class='bi bi-card-checklist'></i> Responsibilities: <?= htmlspecialchars($row['JobDescription']) ?></p>
                                <p class='card-text mx-2'><i class='bi bi-clipboard-check'></i> Requirements: <?= htmlspecialchars($row['qualification']) ?></p>

                                <button type='button' class='btn btn-md text-white' style='background-color: #003c3c;'
                                    onclick="window.location.href='./View/JobApplication.php?job_id=<?= $row['jobOfferId'] ?>'">
                                    Apply
                                </button>

                                <?php $ViewJobId = "viewModal_" . $row['jobOfferId']; // Ensure unique modal ID 
                                ?>
                                <button type='button' class='btn btn-info btn-md' data-bs-toggle='modal'
                                    data-bs-target='#<?= $ViewJobId ?>' onclick='setJobApplicant(<?= $row['jobOfferId'] ?>)'>
                                    View
                                </button>
                            </div>
                        </div>
            ...
            <div class="d-flex justify-content-center my-3">
                <a href="../JobPage/JobOffer/View/AllJobOffers.php" class="btn btn-info">View More</a>
            </div>
        </div>
        </section>
